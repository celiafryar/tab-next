...
It bundles the `CSV_Example` and `superstore_demo_template` reference implementations plus the Superstore case study under `references/`, so the file shapes are on disk rather than described. It also covers the no-code route, Tableau Next's Templates page and Create App, whose silent failures (cross-workspace visualizations dropped from the closure, the semantic model never included, one-app-per-asset claims that survive app deletion) all surface as one misleading `ACCESS_DENIED` on the dashboard; the skill decodes it and gives the recipe that installs. **Origin:** the Salesforce `aftest` template pack, redistributed here rather than XeoMatrix-authored; see `references/PROVENANCE.md` in the skill.
