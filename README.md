...
- **A metric's aggregation must equal its calculation's,** or the metric silently shows nothing. Get it backwards and the tile renders a dash and "couldn't load the metric comparison", while both objects validate individually and the model reports itself valid. Worse, the calculation still works correctly on a worksheet, because only the metric overrides it, so "the calc is fine" doesn't clear the calc.
- **A deploy can run, send a correct payload, and change absolutely nothing.** Deploys are atomic, so one bad property anywhere rejects the whole model, and the extension logs the request but never the response, so the only signal is a toast. The skill carries the recipe: check whether the model's last-modified date actually moved, then replay the logged payload yourself to see the real error. One real case was a metric left cumulative while moving to user aggregation, which blocked a deploy that also carried label, description and Business Preference changes.
