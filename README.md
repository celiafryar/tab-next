...
- **A REST-built model once refused to open in the model editor** (`markup://aura:noAccess`) while validating clean and matching its source on every count; permissions and ownership were ruled out, and the condition cleared by 2026-08-07 without explanation. Until understood, opening the model in the editor is part of migration verification, not an assumption. Re-test on the next REST-built model.
