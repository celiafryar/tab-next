# XeoMatrix Sales Insights - Solution Architecture and Usage
...
|---|---|---|
| `xeomatrix__Sales_Opportunity_Data` | AppFrameworkTemplateBundle | The template: 10 CSVs, 10 data stream definitions, workspace, semantic model, 15 visualizations, 1 dashboard, 48-node create chain |
| `xeomatrix__xmDashboardImage` | LightningComponentBundle | Display-only image widget for the dashboard logo |
| `xeomatrix__APEX_essential_components` | StaticResource | The logo, a 256 x 256 PNG |
...
Install with the standard package URL: `https://login.salesforce.com/packaging/installPackage.apexp?p0=04tQQ00000FxUsfYAF`. Admin-only install is sufficient. The org needs Tableau Next and Data 360 provisioned.
...
3. The Tableau Next dashboard runtime, which instantiates `xeomatrix:xmDashboardImage` in a sandboxed tile.
...
Run against the packaged source (`force-app/main/default/lwc/xmDashboardImage`):

```
grep -rnE "fetch\(|XMLHttpRequest|WebSocket|EventSource|SharedWorker" lwc/xmDashboardImage   # 0
grep -rnE "innerHTML|outerHTML|document\.write|insertAdjacentHTML|DOMParser" lwc/xmDashboardImage   # 0
grep -rnE "localStorage|sessionStorage|indexedDB|document\.cookie" lwc/xmDashboardImage   # 0
grep -rnE "import\(|<script|eval\(" lwc/xmDashboardImage   # 0
grep -rn "runAs" appTemplates/Sales_Opportunity_Data/create-chain.json | grep -vc CurrentUser   # 0
grep -rn '\.\./' appTemplates/Sales_Opportunity_Data   # 0
```
...
Salesforce Code Analyzer 5.14.0, rule selectors `AppExchange` and `Recommended:Security`, run against the exact packaged source of 1.0.0.6: **0 violations** across PMD, ESLint, retire-js, and regex. The flow engine was disabled because the package contains no Flow metadata (see the Code Analyzer notes).
